...
contains
  ...
  subroutine write_plotfile(la,rho,istep,dx,time,prob_lo,prob_hi)

    type(layout)   , intent(in   ) :: la
    type(multifab) , intent(in   ) :: rho
    integer        , intent(in   ) :: istep
    real(dp_t)     , intent(in   ) :: dx,time
    real(dp_t)     , intent(in   ) :: prob_lo(rho%dim), prob_hi(rho%dim)
    ...
    call multifab_copy_c(plotdata(1),1,rho,1,1)
