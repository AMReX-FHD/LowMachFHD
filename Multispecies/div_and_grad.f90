...
contains
  ...
  subroutine compute_grad(mla,rho,gradp,dx, &
  ...
  contains
    ...
    subroutine compute_grad_2d(rho,ng_p,gpx,gpy,ng_g,lo,hi,dx,bc)
      ...
      do j=lo(2),hi(2)
         do i=lo(1),hi(1)+1
            gpx(i,j) = ( rho(i,j)-rho(i-1,j) ) / dx(1)
         end do
      end do
      ...
      if (bc(1,1) .eq. FOEXTRAP .or. bc(1,1) .eq. EXT_DIR) then
         i=lo(1)
         do j=lo(2),hi(2)
            gpx(i,j) = ( rho(i,j)-rho(i-1,j) ) / (0.5d0*dx(1))
         end do
      end if
      ...
      do j=lo(2),hi(2)+1
         do i=lo(1),hi(1)
            gpy(i,j) = ( rho(i,j)-rho(i,j-1) ) / dx(2)
         end do
      end do
      ...
      if (bc(2,1) .eq. FOEXTRAP .or. bc(2,1) .eq. EXT_DIR) then
         j=lo(2)
         do i=lo(1),hi(1)
            gpy(i,j) = ( rho(i,j)-rho(i,j-1) ) / (0.5d0*dx(2))
         end do
      end if
    ...
    subroutine compute_grad_3d(rho,ng_p,gpx,gpy,gpz,ng_g,lo,hi,dx,bc)
      ...
      do k=lo(3),hi(3)
         do j=lo(2),hi(2)
            do i=lo(1),hi(1)+1
               gpx(i,j,k) = ( rho(i,j,k)-rho(i-1,j,k) ) / dx(1)
            end do
         end do
      end do
      ...
      if (bc(1,1) .eq. FOEXTRAP .or. bc(1,1) .eq. EXT_DIR) then
         ...
         do k=lo(3),hi(3)
            do j=lo(2),hi(2)
               gpx(i,j,k) = ( rho(i,j,k)-rho(i-1,j,k) ) / (0.5d0*dx(1))
            end do
         end do
      end if
      if (bc(1,2) .eq. FOEXTRAP .or. bc(1,2) .eq. EXT_DIR) then
      ...
      do k=lo(3),hi(3)
         do j=lo(2),hi(2)+1
            do i=lo(1),hi(1)
               gpy(i,j,k) = ( rho(i,j,k)-rho(i,j-1,k) ) / dx(2)
            end do
         end do
      end do
      ...
      if (bc(2,1) .eq. FOEXTRAP .or. bc(2,1) .eq. EXT_DIR) then
         ...
         do k=lo(3),hi(3)
            do i=lo(1),hi(1)
               gpy(i,j,k) = ( rho(i,j,k)-rho(i,j-1,k) ) / (0.5d0*dx(2))
            end do
         end do
      end if
      if (bc(2,2) .eq. FOEXTRAP .or. bc(2,2) .eq. EXT_DIR) then
      ...
      do k=lo(3),hi(3)+1
         do j=lo(2),hi(2)
            do i=lo(1),hi(1)
               gpz(i,j,k) = ( rho(i,j,k)-rho(i,j,k-1) ) / dx(3)
            end do
         end do
      end do
      ...
      if (bc(3,1) .eq. FOEXTRAP .or. bc(3,1) .eq. EXT_DIR) then
         ...
         do j=lo(2),hi(2)
            do i=lo(1),hi(1)
               gpz(i,j,k) = ( rho(i,j,k)-rho(i,j,k-1) ) / (0.5d0*dx(3))
            end do
         end do
      end if
  ...
  subroutine compute_div(mla,flux_fc,div,dx,start_incomp,start_outcomp,num_comp)

    type(ml_layout), intent(in   ) :: mla
    type(multifab) , intent(in   ) :: flux_fc(:,:)
    ...
    do n = 1,nlevs
       do i = 1, nfabs(div(n))
          pxp => dataptr(flux_fc(n,1), i)
          pyp => dataptr(flux_fc(n,2), i)
          dp => dataptr(div(n), i)
  ...
  contains
    ...
    subroutine compute_div_2d(fluxx,fluxy,ng_p,div,ng_d,dx,lo,hi)

      integer        , intent(in   ) :: lo(:),hi(:),ng_p,ng_d
      real(kind=dp_t), intent(in   ) :: fluxx(lo(1)-ng_p:,lo(2)-ng_p:)
      real(kind=dp_t), intent(in   ) :: fluxy(lo(1)-ng_p:,lo(2)-ng_p:)
    ...
    subroutine compute_div_3d(fluxx,fluxy,fluxz,ng_p,div,ng_d,dx,lo,hi)

      integer        , intent(in   ) :: lo(:),hi(:),ng_p,ng_d
      real(kind=dp_t), intent(in   ) :: fluxx(lo(1)-ng_p:,lo(2)-ng_p:,lo(3)-ng_p:)
      real(kind=dp_t), intent(in   ) :: fluxy(lo(1)-ng_p:,lo(2)-ng_p:,lo(3)-ng_p:)
      real(kind=dp_t), intent(in   ) :: fluxz(lo(1)-ng_p:,lo(2)-ng_p:,lo(3)-ng_p:)
