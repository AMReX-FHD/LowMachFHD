    ...
    if(include_discrete_LMA_correction) then
       ...
       do reaction=1, nreactions
          ...
          do species=1, nspecies
             select case(stoichiometric_factors(specie,1,reaction))
             case(0)
             ...
             case default ! This is essentially impossible in practice and won't happen
                call bl_abort("Stochiometric coefficients larger then 3 not supported")      
             end select
          end do
       end do
       ...
       do reaction=1, nreactions
          ...
          reaction_rates(reaction) = chemical_rates(reaction)*product(n_in(1:nspecies)**stoichiometric_factors(1:nspecies,1,reaction))
       end do
    end if
