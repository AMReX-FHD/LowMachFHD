...
!!!!!!
      ...
      do j=ylo(2),yhi(2)
         do i=ylo(1),yhi(1)
            gpy(i,j) = gpy(i,j) - kc*(phi(i,j)+phi(i,j-1))*0.5*( lap(i,j)-lap(i,j-1) ) * dxinv
         end do
      end do
    ...
    subroutine compute_higher_order_term_3d(phi,ng_p,gpx,gpy,gpz,ng_g,lo,hi,dx,bc,xlo,xhi,ylo,yhi,zlo,zhi)
      ...
      ! laplacian
      do k=xlo(2)-1,xhi(2)+1
        do j=xlo(2)-1,xhi(2)+1
           do i=xlo(1)-1,xhi(1)+1
              lap(i,j) = ( phi(i+1,j,k)-2*phi(i,j,k)+phi(i-1,j,k) + phi(i,j+1,k)-2*phi(i,j,k)+phi(i,j-1,k) &
      ...
      do k=xlo(3),xhi(3)
         do j=xlo(2),xhi(2)
            do i=xlo(1),xhi(1)
               gpx(i,j,k) = gpx(i,j) - kc*0.5*( phi(i,j,k)+phi(i-1,j,k) )*( lap(i,j,k)-lap(i-1,j,k) ) * dxinv
            end do
         end do
      end do
      ...
      do k=ylo(3),yhi(3)
         do j=ylo(2),yhi(2)
            do i=ylo(1),yhi(1)
               gpy(i,j,k) = gpy(i,j) - kc*0.5*( phi(i,j,k)+phi(i,j-1,k) )( lap(i,j,k)-lap(i,j-1,k) ) * dxinv
            end do
         end do
      end do
      ...
      do k=zlo(3),zhi(3)
         do j=zlo(2),zhi(2)
            do i=zlo(1),zhi(1)
               gpz(i,j,k) = gpz(i,j,k) - 0.5*kc*( phi(i,j,k)+phi(i,j,k-1) )*( lap(i,j,k)-lap(i,j,k-1) ) * dxinv
            end do
         end do
      end do
