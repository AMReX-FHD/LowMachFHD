module convert_stag_module
  ...
  public :: average_face_to_cc, average_cc_to_face, average_cc_to_node, &
       average_cc_to_edge, shift_face_to_cc, shift_cc_to_boundary_face
...
!       do i=1,nfabs(cc(n))
  ...
  subroutine shift_cc_to_boundary_face(nlevs,cc,face,start_scomp,start_bccomp,num_comp, &
                                       the_bc_level,increment_bccomp_in)
    ...
    call build(bpt,"shift_cc_to_boundary_face")
    ...
    if (ng_f .ge. ng_c) then
       call bl_error("shift_cc_to_boundary_face requires ng_f < ng_c")
    end if
...
!       do i=1,nfabs(cc(n))
  ...
  end subroutine shift_cc_to_boundary_face
    
  subroutine shift_cc_to_boundary_face_2d(cc,ng_c,facex,facey,ng_f,glo,ghi,adv_bc,xlo,xhi,ylo,yhi)

    use bc_module
    use probin_common_module, only: shift_cc_to_boundary
    ...
    if (adv_bc(1,1) .eq. FOEXTRAP .or. adv_bc(1,1) .eq. HOEXTRAP .or. adv_bc(1,1) .eq. EXT_DIR) then
       do i=xlo(1),xlo(1)+ng_f
          facex(i,xlo(2):xhi(2)) = cc(glo(1),xlo(2):xhi(2))
       end do
    end if
    end if
    if (xhi(1)-ng_f .eq. ghi(1)+1 .and. shift_cc_to_boundary(1,2) .eq. 1) then
    if (adv_bc(1,2) .eq. FOEXTRAP .or. adv_bc(1,2) .eq. HOEXTRAP .or. adv_bc(1,2) .eq. EXT_DIR) then
       do i=xhi(1)-ng_f,xhi(1)
          facex(i,xlo(2):xhi(2)) = cc(ghi(1),xlo(2):xhi(2))
       end do
    end if
    end if
    ...
    ! overwrite y-boundary faces
    ! value in ghost cells represents boundary value
    if (ylo(2)+ng_f .eq. glo(2) .and. shift_cc_to_boundary(2,1) .eq. 1) then
    if (adv_bc(2,1) .eq. FOEXTRAP .or. adv_bc(2,1) .eq. HOEXTRAP .or. adv_bc(2,1) .eq. EXT_DIR) then
       do j=ylo(2),ylo(2)+ng_f
          facey(ylo(1):yhi(1),j) = cc(ylo(1):yhi(1),glo(2))
       end do
    end if
    end if
    if (yhi(2)-ng_f .eq. ghi(2)+1 .and. shift_cc_to_boundary(2,2) .eq. 1) then
    if (adv_bc(2,2) .eq. FOEXTRAP .or. adv_bc(2,2) .eq. HOEXTRAP .or. adv_bc(2,2) .eq. EXT_DIR) then
       do j=yhi(2)-ng_f,yhi(2)
          facey(ylo(1):yhi(1),j) = cc(ylo(1):yhi(1),ghi(2))
       end do
    end if
    end if

  end subroutine shift_cc_to_boundary_face_2d

  subroutine shift_cc_to_boundary_face_3d(cc,ng_c,facex,facey,facez,ng_f,glo,ghi,adv_bc,xlo,xhi,ylo,yhi,zlo,zhi)
    ...
    if (adv_bc(1,1) .eq. FOEXTRAP .or. adv_bc(1,1) .eq. HOEXTRAP .or. adv_bc(1,1) .eq. EXT_DIR) then
       do i=xlo(1),xlo(1)+ng_f
          facex(i,xlo(2):xhi(2),xlo(3):xhi(3)) = &
               cc(glo(1),xlo(2):xhi(2),xlo(3):xhi(3))
       end do
    end if
    end if
    if (xhi(1)-ng_f .eq. ghi(1)+1 .and. shift_cc_to_boundary(1,2) .eq. 1) then
    if (adv_bc(1,2) .eq. FOEXTRAP .or. adv_bc(1,2) .eq. HOEXTRAP .or. adv_bc(1,2) .eq. EXT_DIR) then
       do i=xhi(1)-ng_f,xhi(1)
          facex(i,xlo(2):xhi(2),xlo(3):xhi(3)) = &
               cc(ghi(1),xlo(2):xhi(2),xlo(3):xhi(3))
       end do
    end if
    end if
   ...
   if (adv_bc(2,1) .eq. FOEXTRAP .or. adv_bc(2,1) .eq. HOEXTRAP .or. adv_bc(2,1) .eq. EXT_DIR) then
       do j=ylo(2),ylo(2)+ng_f
          facey(ylo(1):yhi(1),j,ylo(3):yhi(3)) = &
               cc(ylo(1):yhi(1),glo(2),ylo(3):yhi(3))
       end do
    end if
    end if
    ...
    if (adv_bc(2,2) .eq. FOEXTRAP .or. adv_bc(2,2) .eq. HOEXTRAP .or. adv_bc(2,2) .eq. EXT_DIR) then
       do j=yhi(2)-ng_f,yhi(2)
          facey(ylo(1):yhi(1),j,ylo(3):yhi(3)) = &
               cc(ylo(1):yhi(1),ghi(2),ylo(3):yhi(3))
       end do
    end if
    end if

    ! overwrite z-boundary faces
    if (zlo(3)+ng_f .eq. glo(3) .and. shift_cc_to_boundary(3,1) .eq. 1) then
    if (adv_bc(3,1) .eq. FOEXTRAP .or. adv_bc(3,1) .eq. HOEXTRAP .or. adv_bc(3,1) .eq. EXT_DIR) then
       do k=zlo(3),zlo(3)+ng_f
          facez(zlo(1):zhi(1),zlo(2):zhi(2),k) = &
               cc(zlo(1):zhi(1),zlo(2):zhi(2),glo(3))
       end do
    end if
    end if
    if (zhi(3)-ng_f .eq. ghi(3)+1 .and. shift_cc_to_boundary(3,2) .eq. 1) then
    if (adv_bc(3,2) .eq. FOEXTRAP .or. adv_bc(3,2) .eq. HOEXTRAP .or. adv_bc(3,2) .eq. EXT_DIR) then
       do k=zhi(3)-ng_f,zhi(3)
          facez(zlo(1):zhi(1),zlo(2):zhi(2),k) = &
               cc(zlo(1):zhi(1),zlo(2):zhi(2),ghi(3))
       end do
    end if
    end if

  end subroutine shift_cc_to_boundary_face_3d
