...
contains
    ...
    do n=1,nlevs
       ...
       if (dm .eq. 2) then
          ...
          if(filtering_width>0) then
             call filter_stoch_m_flux(mflux_nd_temp(n), filtering_width)
             call filter_stoch_m_flux(mflux_cc_temp(n), filtering_width)
          ...
          if(filtering_width>0) then
             call filter_stoch_m_flux(mflux_ed_temp(n,1), filtering_width)
             call filter_stoch_m_flux(mflux_ed_temp(n,2), filtering_width)
             call filter_stoch_m_flux(mflux_ed_temp(n,3), filtering_width)
             call filter_stoch_m_flux(mflux_cc_temp(n)  , filtering_width)
             call multifab_fill_boundary(mflux_cc_temp(n)) ! First ghost cell is used in divergence
          end if
  ...
  subroutine filter_stoch_m_flux(mfab, width)
    ...
    !       after calling filter_stoch_m_flux!
    ...
    call build(bpt,"filter_stoch_m_flux")
  ...
  end subroutine filter_stoch_m_flux
