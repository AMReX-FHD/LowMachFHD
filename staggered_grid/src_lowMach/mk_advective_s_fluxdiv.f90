...
contains
  ...
  subroutine mk_advective_s_fluxdiv(mla,umac,s_fc,s_update,increment,dx,start_comp,num_comp)
  ...
  contains
    ...
    subroutine mk_advective_s_fluxdiv_2d(sx,sy,umac,vmac,s_update,ng_s,ng_u,ng_a, &
                                         lo,hi,dx,increment)
      ...
      if (increment) then
         do j=lo(2),hi(2)
         do i=lo(1),hi(1)
            s_update(i,j) = s_update(i,j) - ( (fluxx(i+1,j)-fluxx(i,j)) * dxinv &
         ...
         do j=lo(2),hi(2)
         do i=lo(1),hi(1)
            s_update(i,j) = -( (fluxx(i+1,j)-fluxx(i,j)) * dxinv &
                              +(fluxy(i,j+1)-fluxy(i,j)) * dxinv )
         end do
         end do
      end if
    ...
    subroutine mk_advective_s_fluxdiv_3d(sx,sy,sz,umac,vmac,wmac,s_update,ng_s,ng_u,ng_a, &
                                         lo,hi,dx,increment)
      ...
      if (increment) then
         ...
         do j=lo(2),hi(2)
         do i=lo(1),hi(1)
            s_update(i,j,k) = s_update(i,j,k) - ( (fluxx(i+1,j,k)-fluxx(i,j,k)) * dxinv &
         ...
         do j=lo(2),hi(2)
         do i=lo(1),hi(1)
            s_update(i,j,k) =  -( (fluxx(i+1,j,k)-fluxx(i,j,k)) * dxinv &
                                 +(fluxy(i,j+1,k)-fluxy(i,j,k)) * dxinv &
                                 +(fluxz(i,j,k+1)-fluxz(i,j,k)) * dxinv )
         end do
         end do
