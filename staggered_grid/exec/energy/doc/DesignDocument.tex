...
\frac{\partial\rho_i}{\partial t} &=& -\nabla\cdot(\rho_i\vb) - \nabla\cdot\Fb_i + \rho W_i,\label{eq:mass}\\
\frac{\partial\rho h}{\partial t} &=& -\nabla\cdot(\rho h\vb) + \frac{DP}{Dt} - \nabla\cdot\Qb - \sum_k \nabla\cdot h_k \Fb_k + \rho H,\label{eq:enthalpy}
\end{eqnarray}
...
pressure, $h$ is the specific enthalpy, $h_k = \partial h/\partial w_k|_{T,P,w_{j,j\ne k}}$,
$H$ is an external heating term, $\taub$ is the stress tensor,
$\Fb_i$ are the mass fluxes, $W_i$ are external component density
sources/sinks that sum to zero,
...
the evolution of the system is constrained so that the thermodynamic pressure
is constant in space,
\begin{equation}
P_0(t) = P(\rho,\wb,T),\label{eq:EOS}
\end{equation}
where temperature can be computed using $T=T(\rho,\wb,h)$.\\
...
$n_i=\rho_i/m_i$ and $m_i$ the molecular mass of species $i$.
...
We begin by differentiating the unconstrainted equation of state,
$P = P(\rho,T,\wb)$, along particle paths,
\begin{equation}
\frac{DP}{Dt} = P_\rho\frac{D\rho}{Dt} + P_T\frac{DT}{Dt} + \sum_kP_{w_k}\frac{Dw_k}{Dt},\label{eq:particle paths}
\end{equation}
\begin{equation}
P_\rho = \left.\frac{\partial P}{\partial\rho}\right|_{T,\wb},
\quad
P_T = \left.\frac{\partial P}{\partial T}\right|_{\rho,\wb},
\quad
P_{w_k} = \left.\frac{\partial P}{\partial w_k}\right|_{\rho,T,w_{j,j\ne k}}.
\end{equation}
Next, by noting that by continuity $D\rho/Dt = -\rho\nabla\cdot\vb$, 
we can rewrite (\ref{eq:particle paths}) as
\begin{eqnarray}
\nabla\cdot\vb &=& \frac{1}{\rho P_\rho}\left(-\frac{DP}{Dt} + P_T\frac{DT}{Dt} + \sum_kP_{w_k}\frac{Dw_k}{Dt}\right) \nonumber\\
&=& -\frac{\rho_P}{\rho}\frac{DP}{Dt} - \frac{\rho_T}{\rho}\frac{DT}{Dt} - \sum_k\frac{\rho_{w_k}}{\rho}\frac{Dw_k}{Dt} \nonumber\\
&=& -\kappa_T\frac{DP}{Dt} + \alpha\frac{DT}{Dt} - \sum_k\beta_k\frac{Dw_k}{Dt},\label{eq:constraint1}
\end{eqnarray}
where we have used the triple product rule, 
$P_T/P_\rho=-\rho_T$ and $P_{w_k}/P_\rho=-\rho_{w_k}$,
and the standard definitions of isothermal compressibility, thermal expansion coefficient, 
and solutal expansion coefficients,
\begin{equation}
\kappa_T = \frac{\rho_P}{\rho}, \quad
\alpha = -\frac{\rho_T}{\rho}, \quad
\beta_k = \frac{\rho_{w_k}}{\rho}.
\end{equation}
Note that some complex equations of state 
such as Soave-Redlich-Kwong \cite{RedlichKwong,Soave}
are most easily represented in terms of $P=P(\rho,\wb,T)$,
so to find these coefficients we can use the derivatives with
respect to $P$ and the triple product rule.\\

To obtain an expression for $DT/Dt$, we differentiate the enthalpy,
$h=h(T,P,\wb)$, along particle paths to obtain
\begin{equation}
\frac{Dh}{Dt} = 
c_P \frac{DT}{Dt} 
+ h_P\frac{DP}{Dt} + \sum_k h_k\frac{Dw_k}{Dt},
\end{equation}
\begin{equation}
c_P = \left.\frac{\partial h}{\partial T}\right|_{P,\wb},
\quad
h_P = \left.\frac{\partial h}{\partial P}\right|_{T,\wb}.
\end{equation}
This can be rearranged to give
\begin{equation}
\frac{DT}{Dt} = \frac{1}{c_P}\left(\frac{Dh}{Dt} - h_P\frac{DP}{Dt} - \sum_kh_k\frac{Dw_k}{Dt}\right).\label{eq:DTDt1}
\end{equation}
From equation (\ref{eq:mass}) and continuity we have,
\begin{equation}
\frac{Dw_i}{Dt} = \frac{1}{\rho}\left(-\nabla\cdot\Fb_i + \rho W_i\right),\label{eq:DwDt}
\end{equation}
and from equation (\ref{eq:enthalpy}) we have,
\begin{equation}
\frac{Dh}{Dt} = \frac{1}{\rho}\left(\frac{DP}{Dt} - \nabla\cdot\Qb - \sum_k\nabla\cdot h_k\Fb_k + \rho H\right).\label{eq:DhDt}
\end{equation}
Substituting equations (\ref{eq:DwDt}) and (\ref{eq:DhDt}) into (\ref{eq:DTDt1}), we have
\begin{equation}
\frac{DT}{Dt} = \frac{1}{\rho c_P}\left[\left(1 - \rho h_P\right)\frac{DP}{Dt} - \nabla\cdot\Qb - \sum_k\nabla\cdot h_k\Fb_k  + \rho H - \sum_kh_k(-\nabla\cdot\Fb_k + \rho W_k)\right].\label{eq:DTDt}
\end{equation}
Substituting equations (\ref{eq:DTDt}) and (\ref{eq:DwDt}) into (\ref{eq:constraint1}) gives
\begin{equation}
\nabla\cdot\vb + \theta\frac{DP}{Dt} = \frac{\alpha}{\rho c_P}\left[-\nabla\cdot\Qb - \sum_k\nabla\cdot h_k\Fb_k  + \rho H - \sum_k h_k\left(-\nabla\cdot\Fb_k + \rho W_k\right)\right] -\sum_k \frac{\beta_k}{\rho}\left(-\nabla\cdot\Fb_k + \rho W_k\right)\equiv S,\label{eq:constraint}
\end{equation}
\begin{equation}
\theta = \kappa_T - \frac{(1-\rho h_P)\alpha}{\rho c_P}.\label{eq:thermo deriv2}
\end{equation}
...
\end{equation}
Using the identity from \cite{Kuiken94}, $\alpha T = 1 - \rho h_P$, we can say,
\begin{equation}
...
and comparing to (\ref{eq:thermo deriv2}) it is apparent that
\begin{equation}
\theta = \frac{\kappa_T}{\gamma}.
\end{equation}
An alternate derivation can be found in the Appendix of \cite{MAESTROI},
where it was shown that
\begin{equation}
\theta = \frac{1}{\Gamma_1 P},\label{eq:theta_MAESTRO}
\end{equation}
...
coefficient.  The coefficient $\Gamma_1$ is related to the ratio of specific heats, 
$\gamma$, via
\begin{equation}
\Gamma_1 = \gamma\chi_\rho\label{eq:Gamma_1}
\end{equation}
where
\begin{equation}
\chi_\rho \equiv \left(\frac{\partial\ln{P}}{\partial\ln{\rho}}\right)_T = \frac{\rho}{P}P_\rho = \frac{1}{P\kappa_T}\label{eq:chi_rho}
\end{equation}
is the ``density exponent in the pressure equation of state.''  Substituting
equations (\ref{eq:Gamma_1}) and (\ref{eq:chi_rho}) into
(\ref{eq:theta_MAESTRO}), we see that the two definitions of $\theta$ are 
equivalent.\\
...
This model is equivalent to equation (7) in \cite{DayBell:2000} using an ideal
gas equation of state,
\begin{equation}
P = \rho\mathcal{R}T\sum_k\frac{w_k}{m_k}.
\end{equation}
Note that in \cite{DayBell:2000} the substitution
$-\nabla\cdot h_k\Fb_k + h_k\nabla\cdot\Fb_k = -\Fb\cdot\nabla h_k$, has been made.
Also, here the convective derivative of $P$ is no longer zero,
...
except that paper includes a convective derivative of $P_0$ since $P_0$ varies
as a function of latitude due to stratification.
...
\begin{equation}
\nabla\cdot\vb + \theta\frac{dP_0}{dt} = S + \underbrace{\kappa_T\frac{\Delta P}{\Delta t}}_{S_{\rm corr}}.
\end{equation}
...
&& + \half(\rho H)^n + \half(\rho H)^{n+1,l}.\label{eq:temp form}
\end{eqnarray}
\\
...
&&\hspace{-0.5in} + \half(\rho H)^n + \half(\rho H)^{n+1,l}.
\end{eqnarray}
...
\begin{equation}
P(\rho^{n+1},\wb^{n+1},T^{n+1}) = P_0^{n+1},
\end{equation}
...
% $\theta\rightarrow\infty$.  Thus, $P_0$ is constant, and therefore the right-hand-side of equation 
% (\ref{eq:P0_update}) is zero, and the term proportional to $\delta\theta$ in 
...
\begin{itemize}
\item Advance thermodynamic variables $\rho,\wb,h,P_0$.
...
Note that in general that the thermodynamic variables are not
necessarily on the EOS.  The time-advancement consists of:\\ \\
{\bf Step 1:}\\ \\
Compute $(S,\theta,\Fb,\eta,\lambda,h_k)^n$ from $(\rho,\wb,h,P_0,T)^n$.
\MarginPar{The routines in the code take in all these variables and mix the usage.  More details need to be given here.}
Split $(S,\theta)^n$ into average and perturbational components:
\begin{equation}
S^n = \bar{S}^n + \delta S^n,
\end{equation}
\begin{equation}
\theta^n = \bar{\theta}^n + \delta\theta^n.
\end{equation}
{\bf Step 2:}\\ \\
Compute a volume discrepancy correction:
\begin{equation}
S_{\rm corr} = \kappa_T^n\left(\frac{P(\rho^n,\wb^n,T^n) - P_0^n}{\Delta t}\right),
\end{equation}
...
and  $(S,\theta,\Fb,\eta,c_P,\lambda,h_k)^{n+1,m=1} = (S,\theta,\Fb,\eta,c_P,\lambda,h_k)^n$.\\ \\
{\bf Step 3}\\ \\
Split $(S,\theta)^{n+1,1}$ into average and perturbational components:
\begin{equation}
S^{n+1,1} = \bar{S}^{n+1,1} + \delta S^{n+1,1},
\end{equation}
\begin{equation}
\theta^{n+1,1} = \bar{\theta}^{n+1,1} + \delta\theta^{n+1,1}.
\end{equation}
...
&&+ \half(\rho H)^n + \half(\rho H)^{n+1,m+1},
\end{eqnarray}
...
&&\hspace{-1in} + \half(\rho H)^n + \half(\rho H)^{n+1,m+1},
\end{eqnarray}
...
&&\hspace{-1.25in} + \half(\rho H)^n + \half(\rho H)^{n+1,m+1}.\label{eq:deltaT}
\end{eqnarray}
...
\end{equation}
\begin{equation}
\theta^{n+1,m+1} = \bar{\theta}^{n+1,m+1} + \delta\theta^{n+1,m+1},
\end{equation}
...
\end{eqnarray}
\begin{equation}
\nabla\cdot\vb^{n+1,m+1} = \delta S^{n+1,m+1} + \delta S_{\rm corr} - \delta\theta^{n+1,m+1}\left(\frac{\bar{S}^{n+1,m+1} + \bar{S}_{\rm corr}}{\bar{\theta}^{n+1,m+1}}\right).
\end{equation}
...
\begin{equation}
S_{\rm corr} = S_{\rm corr} + 2\kappa_T^{n+1,m+1}\left(\frac{P(\rho^{n+1,m+1},\wb^{n+1,m+1},T^{n+1,m+1}) - P_0^{n+1,m+1}}{\Delta t}\right)
\end{equation}
...
is $w_k = 0.25$.  There is an external heating of $H=10^{10}e^{-100r^2}$~erg/(g-s),
...
\section{Van der Waals Test Problem}

\begin{equation}
P = \frac{\mathcal{R}T}{v-b(\wb)}\sum_k\frac{w_k}{m_k} - \frac{a(\wb)}{v^2}
\end{equation}
with specific volume $v=1/\rho$.  We will denote $T_{c,i}$ and $P_{c,i}$ as the critical
temperature and pressure for component $i$.
\begin{equation}
a(\wb) = \sum_i\sum_j w_iw_j\sqrt{a_i a_j}
\end{equation}
\begin{equation}
a_i = \frac{27}{64}\frac{R^2T_{c,i}^2}{m_i^2 P_{c,i}},
\end{equation}
\begin{equation}
b(\wb) = \sum_i w_i b_i, \quad b_i = \frac{1}{8}\frac{RT_{c,i}}{m_i P_{c,i}}
\end{equation}
...
Variable & $a_i$ & $b_i$ \\
\hline
H$_2$ & $6.08\times 10^{10}$ & 13.2 \\
O$_2$ & $1.35\times 10^9$ & 0.994 \\
N$_2$ & $1.77\times 10^9$ & 1.40 \\
He & $2.16\times 10^9$ & 5.94 \\
Ne & $5.19\times 10^8$ & 0.841 \\
Ar & $8.64\times 10^8$ & 0.816 \\
Kr & $3.35\times 10^8$ & 0.478 \\
\hline
\end{tabular}
\end{center}
\end{table}
...
To evaluate $P_\rho$, use the chain rule, $P_\rho = P_v v_\rho = -v^2 P_v$:
\begin{equation}
P_\rho = \frac{v^2}{(v-b(\wb))^2}\mathcal{R}T\sum_k\frac{w_k}{m_k} - \frac{2a}{v}
\end{equation}
\begin{equation}
P_T = \frac{\mathcal{R}}{v-b(\wb)}\sum_k\frac{w_k}{m_k},
\end{equation}
\begin{equation}
P_{w_k} = \left(\frac{\mathcal{R}T}{v-b(\wb)}\right)\frac{1}{m_k}
+\frac{\mathcal{R}Tb_k\sum_i\frac{w_i}{m_i}}{[v-b(\wb)]^2}
- \frac{\frac{\partial a(\wb)}{\partial w_k}}{v^2}
\end{equation}
with
\begin{equation}
\frac{\partial a(\wb)}{\partial w_k} = 2\sum_i w_i\sqrt{a_ia_k},
\quad
\frac{\partial b(\wb)}{\partial w_k} = b_k.
\end{equation}
