...
module analyze_spectra_module
  ...
  integer   , save :: hydro_grid_int,project_dir,max_grid_projection(2),stats_int,n_steps_save_stats
...
contains   
  ...
  subroutine initialize_hydro_grid(mla,s_in,m_in,dt,dx,namelist_file)
    ...
    nvar = dm + s_in(1)%nc ! mx, my, [mz,] rho, rho*c (or ux, uy, [uz,] rho, c if primitive)
...
!mcai----------start----------------------------
  ...
  subroutine analyze_hydro_grid_serial(mla,s_in,m_in,umac,prim,dt,dx, step, custom_analysis)
    ...
    if(analyze_conserved) then ! Use rho and rho1
       call copy(s_serial,dm+1,s_in(1),1,s_in(1)%nc)
    else ! Use rho and c
       call copy(s_serial,dm+1,prim(1),1,s_in(1)%nc)
    end if   
  ...
  subroutine analyze_hydro_grid_parallel(mla,s_in,m_in,umac,prim,dt,dx,step)
   ...
   !mcai----------end------------------------
    ...
    ! Now gather density and concentration
    if(analyze_conserved) then ! Use rho and rho1
       call copy(s_dir,dm+1,s_in(1),1,s_in(1)%nc)
    else ! Use rho and c
       call copy(s_dir,dm+1,prim(1),1,s_in(1)%nc)
    end if   
