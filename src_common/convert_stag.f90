...
contains
  ...
  subroutine average_cc_to_face_inv(nlevs,cc,face,start_scomp,start_bccomp,num_comp,the_bc_level)
    ...
    if (ng_f .ge. ng_c) then
       call bl_error("average_cc_to_face_inv requires ng_f < ng_c")
    end if
    ...
    do n=1,nlevs
       do i=1,nfabs(cc(n))
          ...
          case (2)
             do scomp=start_scomp,start_scomp+num_comp-1
                bccomp = start_bccomp + scomp - start_scomp
                call average_cc_to_face_inv_2d(cp(:,:,1,scomp),ng_c, &
                                               epx(:,:,1,scomp),epy(:,:,1,scomp),ng_f, &
                                               lo,hi, &
                                               the_bc_level(n)%adv_bc_level_array(i,:,:,bccomp))
             end do
          case (3)
             do scomp=start_scomp,start_scomp+num_comp-1
                bccomp = start_bccomp + scomp - start_scomp
                epz => dataptr(face(n,3), i)
                call average_cc_to_face_inv_3d(cp(:,:,:,scomp),ng_c, &
                                               epx(:,:,:,scomp),epy(:,:,:,scomp),epz(:,:,:,scomp),ng_f, &
                                               lo,hi, &
                                               the_bc_level(n)%adv_bc_level_array(i,:,:,bccomp))
             end do
  ...
  end subroutine average_cc_to_face_inv
    
  subroutine average_cc_to_face_inv_2d(cc,ng_c,facex,facey,ng_f,lo,hi,adv_bc)
    ...
    do j=lo(2)-ng_f,hi(2)+ng_f
       do i=lo(1)-ng_f,hi(1)+ng_f+1
          facex(i,j) = 2.d0 / (cc(i,j)+cc(i-1,j))
       end do
    end do
    ...
    do j=lo(2)-ng_f,hi(2)+ng_f+1
       do i=lo(1)-ng_f,hi(1)+ng_f
          facey(i,j) = 2.d0 / (cc(i,j)+cc(i,j-1))
       end do
    end do
    ...
    if (adv_bc(1,1) .eq. FOEXTRAP .or. adv_bc(1,1) .eq. EXT_DIR) then
       do i=lo(1)-ng_f,lo(1)
          facex(i,lo(2)-ng_f:hi(2)+ng_f) = 1.d0 / cc(lo(1)-1,lo(2)-ng_f:hi(2)+ng_f)
       end do
    end if
    if (adv_bc(1,2) .eq. FOEXTRAP .or. adv_bc(1,2) .eq. EXT_DIR) then
       do i=hi(1)+1,hi(1)+ng_f+1
          facex(i,lo(2)-ng_f:hi(2)+ng_f) = 1.d0 / cc(hi(1)+1,lo(2)-ng_f:hi(2)+ng_f)
       end do
    end if
    ...
    if (adv_bc(2,1) .eq. FOEXTRAP .or. adv_bc(2,1) .eq. EXT_DIR) then
       do j=lo(2)-ng_f,lo(2)
          facey(lo(1)-ng_f:hi(1)+ng_f,j) = 1.d0 / cc(lo(1)-ng_f:hi(1)+ng_f,lo(2)-1)
       end do
    end if
    if (adv_bc(2,2) .eq. FOEXTRAP .or. adv_bc(2,2) .eq. EXT_DIR) then
       do j=hi(2)+1,hi(2)+ng_f+1
          facey(lo(1)-ng_f:hi(1)+ng_f,j) = 1.d0 / cc(lo(1)-ng_f:hi(1)+ng_f,hi(2)+1)
       end do
    end if

  end subroutine average_cc_to_face_inv_2d

  subroutine average_cc_to_face_inv_3d(cc,ng_c,facex,facey,facez,ng_f,lo,hi,adv_bc)
    ...
    do k=lo(3)-ng_f,hi(3)+ng_f
       do j=lo(2)-ng_f,hi(2)+ng_f
          do i=lo(1)-ng_f,hi(1)+ng_f+1
             facex(i,j,k) = 2.d0 / (cc(i,j,k)+cc(i-1,j,k))
          end do
       end do
    end do
    ...
    do k=lo(3)-ng_f,hi(3)+ng_f
       do j=lo(2)-ng_f,hi(2)+ng_f+1
          do i=lo(1)-ng_f,hi(1)+ng_f
             facey(i,j,k) = 2.d0 / (cc(i,j,k)+cc(i,j-1,k))
          end do
       end do
    end do
    ...
    do k=lo(3)-ng_f,hi(3)+ng_f+1
       do j=lo(2)-ng_f,hi(2)+ng_f
          do i=lo(1)-ng_f,hi(1)+ng_f
             facez(i,j,k) = 2.d0 / (cc(i,j,k)+cc(i,j,k-1))
          end do
       end do
    end do
    ...
    if (adv_bc(1,1) .eq. FOEXTRAP .or. adv_bc(1,1) .eq. EXT_DIR) then
       do i=lo(1)-ng_f,lo(1)
          facex(i,lo(2)-ng_f:hi(2)+ng_f,lo(3)-ng_f:hi(3)+ng_f) = &
               1.d0 / cc(lo(1)-1,lo(2)-ng_f:hi(2)+ng_f,lo(3)-ng_f:hi(3)+ng_f)
       end do
    end if
    if (adv_bc(1,2) .eq. FOEXTRAP .or. adv_bc(1,2) .eq. EXT_DIR) then
       do i=hi(1)+1,hi(1)+ng_f+1
          facex(i,lo(2)-ng_f:hi(2)+ng_f,lo(3)-ng_f:hi(3)+ng_f) = &
               1.d0 / cc(hi(1)+1,lo(2)-ng_f:hi(2)+ng_f,lo(3)-ng_f:hi(3)+ng_f)
       end do
    end if
    ...
    if (adv_bc(2,1) .eq. FOEXTRAP .or. adv_bc(2,1) .eq. EXT_DIR) then
       do j=lo(2)-ng_f,lo(2)
          facey(lo(1)-ng_f:hi(1)+ng_f,j,lo(3)-ng_f:hi(3)+ng_f) = &
               1.d0 / cc(lo(1)-ng_f:hi(1)+ng_f,lo(2)-1,lo(3)-ng_f:hi(3)+ng_f)
       end do
    end if
    if (adv_bc(2,2) .eq. FOEXTRAP .or. adv_bc(2,2) .eq. EXT_DIR) then
       do j=hi(2)+1,hi(2)+ng_f+1
          facey(lo(1)-ng_f:hi(1)+ng_f,j,lo(3)-ng_f:hi(3)+ng_f) = &
               1.d0 / cc(lo(1)-ng_f:hi(1)+ng_f,hi(2)+1,lo(3)-ng_f:hi(3)+ng_f)
       end do
    end if
    ...
    if (adv_bc(3,1) .eq. FOEXTRAP .or. adv_bc(3,1) .eq. EXT_DIR) then
       do k=lo(3)-ng_f,lo(3)
          facez(lo(1)-ng_f:hi(1)+ng_f,lo(2)-ng_f:hi(2)+ng_f,k) = &
               1.d0 / cc(lo(1)-ng_f:hi(1)+ng_f,lo(2)-ng_f:hi(2)+ng_f,lo(3)-1)
       end do
    end if
    if (adv_bc(3,2) .eq. FOEXTRAP .or. adv_bc(3,2) .eq. EXT_DIR) then
       do k=hi(3)+1,hi(3)+ng_f+1
          facez(lo(1)-ng_f:hi(1)+ng_f,lo(2)-ng_f:hi(2)+ng_f,k) = &
               1.d0 / cc(lo(1)-ng_f:hi(1)+ng_f,lo(2)-ng_f:hi(2)+ng_f,hi(3)+1)
       end do
    end if

  end subroutine average_cc_to_face_inv_3d
