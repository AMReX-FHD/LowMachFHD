...
contains
  ...
  subroutine bds_quad(mla,umac,s,s_update,force,dx,dt,start_comp,num_comp)
    ...
    do i = 1, nfabs(s(lev))
       ...
       case (2)
          do n = start_comp, start_comp+num_comp-1
             call bdsslope_quad_2d(lo, hi, sop(:,:,1,n), ng, &
                                   avep(:,:,1,1), slxp(:,:,1,1), slyp(:,:,1,1), &
                                   slxyp(:,:,1,1), slxxp(:,:,1,1), slyyp(:,:,1,1), &
                                   sip(:,:,1,1), scp(:,:,1,:), dx(lev,:)) 

             call  bdsconc_quad_2d(lo, hi, sop(:,:,1,n), ng, snp(:,:,1,n), ng_u, &
                                   fp(:,:,1,n), ng_f, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine bdsconc_quad_2d(lo,hi,s,ng,s_update,ng_u,force,ng_f, &
