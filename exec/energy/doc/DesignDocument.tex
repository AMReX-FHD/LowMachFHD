...
Set $(\vb,\rho,\wb,h,T,P_0)^{n+1} = (\vb,\rho,\wb,h,T,P_0)^n$ and
iterate over an outer loop over {\bf Steps 3-8}:\\ \\
{\bf Step 3}\\ \\
Compute $(S,\alpha,\Fb,\eta,c_p,\lambda,h_k)^{n+1}$ 
from $(\rho,\wb,h,T,P_0)^{n+1}$.
We note that $(S,\alpha,\Fb,\eta)^{n+1}$ will stay fixed through the
current iteration over {\bf Steps 3-8}, whereas $(c_p,\lambda,h_k)$ are
recomputed as part of the iterative enthalpy solve ({\bf Step 6}).
Also, $\rho, \wb, h, T$, and $\vb$ are updated sequentially and the 
updated values are used as needed.
...
\begin{eqnarray}
\frac{\rho^{n+1}(h^{n+1,l} + c_p^{n+1,l}\delta T) - (\rho h)^n}{\Delta t} &=& \half\left[-\nabla\cdot(\rho h\vb)^n -\nabla\cdot(\rho h\vb)^{n+1} + \frac{\bar{S}^n + \bar{S}_{\rm corr}}{\bar{\alpha}^n} + \frac{\bar{S}^{n+1}+ \bar{S}_{\rm corr}}{\bar{\alpha}^{n+1}}\right]\nonumber\\
&&\hspace{-1in} + \half\nabla\cdot\lambda^n\nabla T^n + \half\nabla\cdot\lambda^{n+1,l}\nabla(T^{n+1,l} + \delta T)\nonumber\\
...
\begin{eqnarray}
\left(\frac{\rho^{n+1}c_p^{n+1,l}}{\Delta t} - \half\nabla\cdot\lambda^{n+1}\nabla\right)\delta T &=& \frac{(\rho h)^n - \rho^{n+1}h^{n+1,l}}{\Delta t}\nonumber\\
&&\hspace{-1.25in} + \half\left[-\nabla\cdot(\rho h\vb)^n - \nabla\cdot(\rho h\vb)^{n+1} + \frac{\bar{S}^n + \bar{S}_{\rm corr}^n}{\bar{\alpha}^n} + \frac{\bar{S}^{n+1} + \bar{S}_{\rm corr}^{n+1}}{\bar{\alpha}^{n+1}}\right]\nonumber\\
&&\hspace{-1.25in} + \half\nabla\cdot\lambda^n\nabla T^n + \half\nabla\cdot\lambda^{n+1}\nabla T^{n+1,l}\nonumber\\
&&\hspace{-1.25in} + \half\sum_k\nabla\cdot h_k^n \Fb_k^n + \half\sum_k\nabla\cdot h_k^{n+1,l} \Fb_k^{n+1}\nonumber\\
&&\hspace{-1.25in} + \half(\rho\Hext)^n + \half(\rho\Hext)^{n+1}.\label{eq:deltaT}
\end{eqnarray}
To proceed, we set $(h,T)^{n+1,l=1} = (h,T)^{n+1}$ and iterate over an inner 
loop from $l=1$ over {\bf Steps 6a-c}:\\ \\
{\bf Step 6a}\\ \\
Compute $(c_p,\lambda,h_k)^{n+1,l}$ from $(\rho^{n+1},\wb^{n+1},T^{n+1,l})$.\\ \\
{\bf Step 6b}\\ \\
...
{\bf Step 7}\\ \\
GMRES Solve for $\vb^{n+1,k+1}$ and $\pi^{n+1}$.
\MarginPar{What do we use to compute $\eta^{n+1}$?}
\begin{equation}
\frac{(\rho\vb)^{n+1,k+1} - (\rho\vb)^n}{\Delta t} + \nabla\pi^{n+1} = \half\left[-\nabla\cdot(\rho\vb\vb)^n - \nabla\cdot(\rho\vb\vb)^{n+1,k}\right] + \frac{1}{2}\nabla\cdot(\taub^n + \taub^{n+1,k+1}) + \frac{1}{2}(\rho^n + \rho^{n+1})\gb,
\end{equation}
\begin{equation}
\nabla\cdot\vb^{n+1,k+1} = \delta S^{n+1} + \delta S_{\rm corr} - \delta\alpha^{n+1}\left(\frac{\bar{S}^{n+1} + \bar{S}_{\rm corr}}{\bar{\alpha}^{n+1}}\right).
\end{equation}
If we express the viscous stress tensor in operator form, e.g., 
$\mathcal{A}_0^n\vb = \nabla\cdot[\eta^n(\nabla\vb + (\nabla\vb)^T)]$, 
...
expressed in a familiar Stokes operator form,
\begin{eqnarray}
\left(\frac{\rho^{n+1}}{\Delta t} - \half\mathcal{A}_0^{n+1}\right)\vb^{n+1,k+1} + \nabla\pi^{n+1} &=& \frac{\rho^n\vb^n}{\Delta t} - \half\nabla\cdot\left[(\rho\vb\vb)^n + (\rho\vb\vb)^{n+1,k}\right]\nonumber\\
...
In practice, we solve for a modified increment to the velocity field, $\deltab\vb$.  In particular,
we split the updated velocity into
$\vb^{n+1,k+1} = \bar{\vb}^n + \deltab\vb$, where $\bar{\vb}^n$ is the velocity at $t^n$, but with modified boundary
...
\begin{eqnarray}
\left(\frac{\rho^{n+1}}{\Delta t} - \half\mathcal{A}_0^{n+1}\right)\deltab\vb + \nabla\delta\pi &=& \frac{\rho^n\vb^n - \rho^{n+1}\bar{\vb}^n}{\Delta t} - \nabla\pi^n\nonumber\\
&&\hspace{-1.5in} - \half\left[\nabla\cdot(\rho\vb\vb)^n + \nabla\cdot(\rho\vb\vb)^{n+1,k}\right] + \half(\mathcal{A}_0^n\vb^n+\mathcal{A}_0^{n+1}\bar{\vb}^n) + \half(\rho^n + \rho^{n+1})\gb,\nonumber\\
\end{eqnarray}
\begin{equation}
-\nabla\cdot\delta\vb = \nabla\cdot\bar{\vb}^n - \left[\delta S^{n+1} + \delta S_{\rm corr} - \delta\alpha^{n+1}\left(\frac{\bar{S}^{n+1} + \bar{S}_{\rm corr}}{\bar{\alpha}^{n+1}}\right)\right].
\end{equation}
{\bf Step 8}\\ \\
