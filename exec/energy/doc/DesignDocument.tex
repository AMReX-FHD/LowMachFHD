...
\begin{equation}
\nabla\cdot\vb + \alpha\frac{dP_0}{dt} = S + \underbrace{\frac{1}{P}\frac{\Delta P}{\Delta t}}_{S_{\rm corr}}.
\end{equation}
...
\begin{equation}
S_{\rm corr}^n = S_{\rm corr}^n + \frac{1}{P_0^n}\left(\frac{P_{\rm EOS}^n - P_0^n}{\Delta t}\right),
\end{equation}
...
\begin{equation}
S_{\rm corr}^n = S_{\rm corr}^n + \frac{1}{P_0^{*,n+1}}\left(\frac{P_{\rm EOS}^{*,n+1} - P_0^{*,n+1}}{\Delta t}\right),
\end{equation}
...
update the volume discrepancy correction,
\begin{equation}
S_{\rm corr}^{*,n+1} = S_{\rm corr}^{*,n+1} + \frac{2}{P_0^{n+1}}\left(\frac{P_{\rm EOS}^{n+1} - P_0^{n+1}}{\Delta t}\right),
\end{equation}
...
Compute a volume discrepancy correction:
\begin{equation}
S_{\rm corr} = \frac{1}{P_0^n}\left(\frac{P(\rho,\wb,T)^n - P_0^n}{\Delta t}\right),
\end{equation}
...
\MarginPar{What do we use to compute $\eta^{n+1}$?  And can the advective term mix $m$ and $m+1$ states?}
\begin{equation}
...
&& + \half\mathcal{A}_0^n\vb^n + \half(\rho^n + \rho^{n+1,m+1})\gb.
\end{eqnarray}
\MarginPar{These notes about solving in homogeneous delta form don't really have to make it into the paper.
They are more to help me with implementation.
We could mention that we do it, and just cite the binary paper.}
