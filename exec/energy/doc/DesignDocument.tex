...
\begin{equation}
\nabla\cdot\vb + \theta\frac{DP}{Dt} = \sum_k \frac{1}{\rho}\beta_k \nabla\cdot\Fb_k + \frac{1}{\rho c_p}\alpha\left[\nabla\cdot\Qb + \sum_k\left(\nabla\cdot h_k\Fb_k - h_k\nabla\cdot\Fb_k + h_kE_k\right) - \rho\Hext\right] \equiv S,\label{eq:constraint}
\end{equation}
\begin{equation}
\theta = \frac{1}{\rho P_\rho} + \frac{(1-\rho h_p)}{\rho c_p}\alpha, \quad
...
Some complex equations of state are represented in terms of $P=P(\rho,w_k,T)$,
...
\begin{equation}
\theta = \frac{1}{\gamma\rho P_\rho} = \frac{1}{\gamma P}
\end{equation}
...
\begin{equation}
\beta_k = -\frac{\rho\bar{m}}{m_k}, \qquad
\alpha = -\frac{\rho}{T}
\end{equation}
