...
We are given $(\vb,\rho,\wb,h,P_0)^{\rm init}$.
Compute a volume discrepancy correction:
\begin{equation}
...
Project:
\begin{equation}
\nabla\cdot\vb^0 = S^{\rm init} + S_{\rm corr}.
\end{equation}
Set $\pi^0=0$.

\subsection{Time-Advancement Scheme}
We are given $(\vb,\rho,\wb,h,P_0,\pi)^n$.
Note in general that these variables are not in exact thermodynamic equilibrium.
Compute $T^n = T(\rho,\wb,h)^n$.
Initialize $(\vb,\rho,\wb,h)^{n+1} = (\vb,\rho,\wb,h)^n$.
...
\end{equation}
{\bf Step 6}\\ \\
Recompute enthalpy semi-implicitly.  This involves a linearized iteration for $\delta T$ (details coming).
But the fundamental discretization is
\begin{eqnarray}
...
\end{eqnarray}
{\bf Step 7}\\ \\
GMRES Solve for $\vb^{n+1}$ and $\pi^{n+1}$.  Question for John: should we recompute
$\rho^{n+1} = \rho^n - \Delta t\nabla\cdot(\rho\vb)^n$ here if this is the first time through the iteration?
\begin{equation}
