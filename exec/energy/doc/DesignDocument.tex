...
\begin{eqnarray}
\frac{DT}{Dt} &=& \frac{1}{\rho c_p}\left[\left(1 - \rho h_P\right)\frac{dP_0}{dt} + \nabla\cdot\Qb + \sum_k\nabla\cdot h_k\Fb_k + \rho\Hext - \sum_kh_k\nabla\cdot\Fb_k\right]\nonumber\\
&=& \frac{1}{\rho c_p}\left[\left(1 - \rho h_P\right)\frac{dP_0}{dt} + \nabla\cdot\Qb + \sum_k\Fb_k\cdot\nabla h_k + \rho\Hext\right].\label{eq:DTDt}
\end{eqnarray}
Combining equations (\ref{eq:constraint1}) and (\ref{eq:DTDt}) gives
\footnote{This is the same model used in Day/Bell, {\it Combust.~Theor.~Model.}, 2000, except now there is a time-derivative of $p_0$.  The coefficient in front of the pressure derivative matches that in MAESTRO Paper III (Almgren et al., {\it Astrophys. J.}, 2008) except that paper includes a convective derivative of $P_0$ due to stratification.}
\begin{equation}
\nabla\cdot\vb + \alpha\frac{dP_0}{dt} = \sum_k \beta_k \nabla\cdot\Fb_k + \theta\left(\nabla\cdot\Qb + \sum_k\Fb_k\cdot\nabla h_k + \rho\Hext\right) \equiv S,\label{eq:constraint}
\end{equation}
\begin{equation}
\alpha = -\frac{1}{\rho^2 P_\rho}\left[\frac{(1-\rho h_p)P_T - \rho c_p}{c_p}\right], \quad
\beta_k = \frac{P_{w_k}}{\rho^2 P_\rho}, \quad
...
&& + \half\sum_k\nabla\cdot h_k^n \Fb_k^n + \half\sum_k\nabla\cdot h_k^{n+1,l}\Fb_k^{n+1,l}.\label{eq:temp form}
\end{eqnarray}
\\
...
\begin{equation}
(\rho h)^{*,n+1} = (\rho h)^n + \Delta t\left[-\nabla\cdot(\rho h\vb)^n + \frac{\bar{S}^n + \bar{S}_{\rm corr}^n}{\bar{\alpha}^n} + \frac{1}{2}(\nabla\cdot\Qb^n + \nabla\cdot\Qb^{*,n+1})\right],
\end{equation}
...
\begin{equation}
(\rho h)^{n+1} = (\rho h)^n + \frac{\Delta t}{2}\left[-\nabla\cdot(\rho h\vb)^n -\nabla\cdot(\rho h\vb)^{*,n+1} + \frac{\bar{S}^n + \bar{S}_{\rm corr}^n}{\bar{\alpha}^n} + \frac{\bar{S}^{*,n+1}+ \bar{S}_{\rm corr}^{*,n+1}}{\bar{\alpha}^{*,n+1}} + \nabla\cdot\Qb^n + \nabla\cdot\Qb^{n+1}\right],
\end{equation}
...
(denoted $m_k$ in our multispecies paper; in these notes we use the conventions in Day/Bell, {\it Combust.~Theor.~Model.}, 2000).
The enthalpy, mass fractions, and temperature are related by:
\begin{equation}
h = \sum_k Y_k h_k(T).
\end{equation}
Recall equations (\ref{eq:constraint}) and (\ref{eq:thermo deriv}):
\begin{equation}
\nabla\cdot\vb + \alpha\frac{dP_0}{dt} = \sum_k \beta_k \nabla\cdot\Fb_k + \theta\left(\nabla\cdot\Qb + \sum_k\Fb_k\cdot\nabla h_k + \rho\Hext\right) \equiv S,
\end{equation}
\begin{equation}
\alpha = -\frac{1}{\rho^2 P_\rho}\left[\frac{(1-\rho h_p)P_T - \rho c_p}{c_p}\right], \quad
\beta_k = \frac{P_{w_k}}{\rho^2 P_\rho}, \quad
\theta = \frac{1}{\rho^2 P_\rho}\frac{P_T}{c_P}.
\end{equation}
For an ideal gas, the thermodynamic derivatives reduce to,
\begin{eqnarray}
P_\rho = \mathcal{R} T\sum_k\frac{w_k}{W_k}, \quad
P_T = \rho\mathcal{R}\sum_k\frac{w_k}{W_k}, \quad
P_{w_k} = \frac{\rho\mathcal{R}T}{W_k},
\end{eqnarray}
and simply the coefficients in the constraint equation,
\begin{equation}
\beta_k = \frac{1}{\rho}\frac{W}{W_k}, \quad
...
\begin{equation}
\alpha = \frac{1}{\Gamma_1 p}, \quad \Gamma_1 = \left(\frac{d\ln{p}}{d\ln{\rho}}\right)_{\rm ad},
\end{equation}
which for an ideal gas simplifies to
\begin{equation}
\alpha = \frac{1}{\gamma P}.
\end{equation}
...
\item {\tt CKCPBS} computes $c_p(\wb,T)$.  For now this is not actually a function
of temperature.\\

\item {\tt CKCVBS} computes $c_v(\wb,T)$.  For now this is not actually a function
of temperature.\\
...
\item {\tt CKHMS} computes $h_m(\wb,T)$.  Note $h = \sum_k w_k h_k$.\\

\item {\tt CKYTX} computes mole fractions, $\xb$, from mass fractions, $\wb$.\\

\item {\tt CKYTCR} computes molar concentrations from $\rho,\wb$.\\
...
$c_p(T)$.  For now this is not actually a function of temperature.\\
...
\item {\tt zeta} is the bulk viscosity, $\kappa$, which we will not use for now.\\

\item {\tt diff\_ij} is the diffusion matrix, $\chi$.\\

\item {\tt kappa} is the thermal conductivity, $\lambda$.\\

\item {\tt chitil} is related to the thermodiffusion coefficients, 
such that $\zeta_i = x_i\tilde{\chi}_i$\\
