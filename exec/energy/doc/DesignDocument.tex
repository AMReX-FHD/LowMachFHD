...
Otherwise,
compute $(\lambda,c_p,h_k)^{n+1,m+1,l+1}$ from $(\rho^{n+1,m+1},\wb^{n+1,m+1},T^{n+1,m+1,l+1})$
and return to {\bf Step 6a}.\\ \\
{\bf Step 7}\\ \\
...
\end{equation}
The perform a GMRES solve for $(\vb,\pi)^{n+1,m+1}$.
\MarginPar{What do we use to compute $\eta^{n+1}$?}
\begin{equation}
\frac{(\rho\vb)^{n+1,m+1} - (\rho\vb)^n}{\Delta t} + \nabla\pi^{n+1,m+1} = \half\left[-\nabla\cdot(\rho\vb\vb)^n - \nabla\cdot(\rho\vb\vb)^{n+1,m}\right] + \frac{1}{2}\nabla\cdot(\taub^n + \taub^{n+1,m+1}) + \frac{1}{2}(\rho^n + \rho^{n+1,m+1})\gb,
\end{equation}
\begin{equation}
\nabla\cdot\vb^{n+1,m+1} = \delta S^{n+1,m+1} + \delta S_{\rm corr} - \delta\alpha^{n+1}\left(\frac{\bar{S}^{n+1} + \bar{S}_{\rm corr}}{\bar{\alpha}^{n+1}}\right).
\end{equation}
If we express the viscous stress tensor in operator form, e.g., 
$\mathcal{A}_0^n\vb = \nabla\cdot[\eta^n(\nabla\vb + (\nabla\vb)^T)]$, 
...
\begin{eqnarray}
\left(\frac{\rho^{n+1}}{\Delta t} - \half\mathcal{A}_0^{n+1}\right)\deltab\vb + \nabla\delta\pi &=& \frac{\rho^n\vb^n - \rho^{n+1}\bar{\vb}^n}{\Delta t} - \nabla\pi^n\nonumber\\
&&\hspace{-1.5in} - \half\left[\nabla\cdot(\rho\vb\vb)^n + \nabla\cdot(\rho\vb\vb)^{n+1,m}\right] + \half(\mathcal{A}_0^n\vb^n+\mathcal{A}_0^{n+1}\bar{\vb}^n) + \half(\rho^n + \rho^{n+1})\gb,\nonumber\\
\end{eqnarray}
\begin{equation}
