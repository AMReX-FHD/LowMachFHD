...
The densities are $\rho_i = \rho w_i$, with $w_i$ the concentrations (mass fractions).
...
\end{equation}
where $\xb$ are the mole fractions.
...
I suggest we specify $\rho,\rho_i$, and $P_0$ and then use the EOS to compute a consistent
...
\item {\tt zeta} is the bulk viscosity, which we will not use for now.\\
