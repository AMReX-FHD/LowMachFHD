...
(denoted $m_k$ in our multispecies paper; in these notes we use the conventions in 
\cite{DayBell:2000}.
...
\begin{equation}
\nabla\cdot\vb + \alpha\frac{dP_0}{dt} = \sum_k \beta_k \nabla\cdot\Fb_k + \theta\left(\nabla\cdot\Qb + \sum_k\nabla\cdot h_k\Fb_k + \rho\Hext\right) \equiv S,
\end{equation}
\begin{equation}
\alpha = -\frac{1}{\rho^2 P_\rho}\left[\frac{(1-\rho h_p)P_T - \rho c_p}{c_p}\right], \quad
\beta_k = \beta_k = \frac{P_{w_k}}{\rho^2 P_\rho} - \frac{h_k P_T}{\rho^2 c_p P_\rho}, \quad
...
\begin{equation}
\beta_k = \frac{1}{\rho}\left(\frac{W}{W_k} - \frac{h_m}{c_p T}\right), \quad
