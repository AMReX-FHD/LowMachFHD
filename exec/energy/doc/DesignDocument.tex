...
the equation of state, $T = T(\rho,\wb,h)$, or the enthalpy using $h = h(\rho,P,\wb)$.
...
to $P_0$.  Since in our low Mach number mode $P_0$ is spatially constant (and 
temporally constant as well for open containers), we can enforce that the 
thermodynamic pressure remain equal to $P_0$ by replacing $DP/Dt$ 
with $\partial P_0/\partial t$:
\begin{equation}
...
substitution $DP/Dt = \partial P_0/\partial t$.  Also, since we
...
\end{equation}
To obtain an expression for $DT/Dt$, we differentiate the enthalpy, $h=h(P,T,\wb)$, 
