...
&=& -\rho\Wb\chib\Gammab\nabla\xb - \underbrace{\frac{\rho^2}{nk_B T}\Wb\chib\Wb\zb}_{A_\Phi}\nabla\Phib
\end{eqnarray}
where $A_\Phi$ is the electrostatic mass flux coefficient.
...
\begin{equation}
\rho^{*,n+1}\zb^T\wb^{*,n+1} - \Delta t\theta\zb^T\nabla\cdot A_\Phi^n\nabla\Phib^{*,n+1} = \zb^T\left(\rho^n\wb^n + \Delta t\mathcal{R}_p\right)
\end{equation}
Substituting this into equation (\ref{eq:constraint}) gives
\begin{equation}
-\nabla\cdot\left(\epsilon + \Delta t\theta\zb^T A_\Phi^n\right)\nabla\Phib^{*,n+1} = \zb^T\left(\rho^n\wb^n + \Delta t\mathcal{R}_p\right),
\end{equation}
which we solve for $\Phib^{*,n+1}$.  Note that
\begin{equation}
\zb^T A_\Phi = \frac{\rho^2}{nk_BT}\zb^T\Wb\chib\Wb\zb,
\end{equation}
...
We would like to solve the following,
\begin{eqnarray}
\frac{\rho^{n+1}\wb^{n+1} - \rho^n\wb^n}{\Delta t} + \half\nabla\cdot(\rho\wb\vb)^n + \half\nabla\cdot(\rho\wb\vb)^{*,n+1} &=&\half\nabla\cdot(\rho\Wb\chib\Gammab\nabla\xb)^n + \half\nabla\cdot(\rho\Wb\chib\Gammab\nabla\xb)^{*,n+1} \nonumber\\
&&\hspace{-2in} + \theta\nabla\cdot A_\Phi^{*,n+1}\nabla\Phib^{n+1} +
...
\begin{eqnarray}
\phi^{n+1}\wb^{n+1} - \Delta t\theta\nabla\cdot A_\Phi^{*,n+1}\nabla\Phib^{n+1} &=& \rho^n\wb^n\nonumber\\
...
\begin{equation}
\rho^{n+1}\zb^T\wb^{n+1} - \Delta t\theta\zb^T\nabla\cdot A_\Phi^{*,n+1}\nabla\Phib^{n+1} = \zb^T(\rho^n\wb^n + \Delta t\mathcal{R}_c).
\end{equation}
Substituting this into equation (\ref{eq:constraint2}) gives
\begin{equation}
-\nabla\cdot\left(\epsilon + \Delta t\theta\zb^T A_\Phi^{*,n+1}\right)\nabla\Phib^{n+1} = \zb^T\left(\rho^n\wb^n + \Delta t\mathcal{R}_c\right),
\end{equation}
which we solve for $\Phib^{n+1}$.  Once we have $\Phib^{n+1}$, we can compute 
$\wb^{n+1}$ using (\ref{eq:implicit phi3}).\\ \\
