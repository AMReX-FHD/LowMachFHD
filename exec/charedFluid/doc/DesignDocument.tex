...
Thus, we can define the electrostatic mass flux coefficient,
\begin{eqnarray}
...
\begin{eqnarray}
\rho^{*,n+1}\wb^{*,n+1} - \Delta t\theta\nabla\cdot A^n\nabla\phi^{*,n+1} &=& \nonumber\\
&&\hspace{-1.5in}\rho^n\wb^n + \Delta t\underbrace{\left[-\nabla\cdot(\rho\wb\vb)^n + \nabla\cdot(\rho\Wb\chib\Gammab\nabla\xb)^n + (1-\theta) \nabla\cdot A^n\nabla\phi^n\right]}_{\mathcal{R}^n}\label{eq:implicit phi}
\end{eqnarray}
\begin{equation}
...
\begin{equation}
\rho^{*,n+1}\zb^T\wb^{*,n+1} - \Delta t\theta\zb^T\nabla\cdot A^n\nabla\phi^{*,n+1} = \zb^T\left(\rho^n\wb^n + \Delta t\mathcal{R}^n\right)
\end{equation}
...
Note that
\begin{equation}
\zb^T A = \frac{\rho^2}{nk_BT}\zb^T\Wb\chib\Wb\zb,
\end{equation}
