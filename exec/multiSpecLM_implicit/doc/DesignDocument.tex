...
We modify the initial configuration of the MMI problem slightly so we instead model
diffusive layer convection (DLC).
We analyze the spectra of the vertically-averaged density, as is done in shadowgraph
experiments.  The setup remains similar to the MMI setup, but the 
concentrations of salt salute and sucrose solutes are modified so that we initially have
a {\it light} solution ``A'' consisting of a salt solute (KCl) diluted with water sitting upon
a {\it heavier} solution ``B'' consisting of a sucrose solute diluated with water.  
As before, fluid ``1'' is salt solute, fluid ``2'' is sugar solute, and fluid ``3'' is water.
