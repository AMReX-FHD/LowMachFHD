...
\begin{itemize}
\item $|${\tt visc\_type}$|$=1 $\quad\rightarrow\quad \mathcal{A}_0\vb \equiv \nabla\cdot\beta\nabla\vb$.\\
\item $|${\tt visc\_type}$|$=2 $\quad\rightarrow\quad \mathcal{A}_0\vb \equiv \nabla\cdot\left\{\beta[\nabla\vb + (\nabla\vb)^T]\right\}$.\\
\item $|${\tt visc\_type}$|$=3 $\quad\rightarrow\quad \mathcal{A}_0\vb \equiv \nabla\cdot\left\{\beta[\nabla\vb + (\nabla\vb)^T] + \mathcal{I}\left(\gamma - \frac{2}{3}\beta\right)(\nabla\cdot\vb)\right\}$.\\
\end{itemize}
...
\begin{equation}
\frac{\rho^{*,n+1}\vb^{*,n+1} - \rho^n\vb^n}{\Delta t} + \nabla\pi = \nabla\cdot(-\rho^n\vb^n\vb^n) + \half\left(\mathcal{A}_0\vb^n + \mathcal{A}_0\vb^{*,n+1}\right) + \nabla\cdot\underbrace{\sqrt{\frac{2\eta^n k_B T}{\Delta t\Delta V}}\widehat\Wb^n}_{\Sigmab^n},\label{eq:CN Vel Pred}
\end{equation}
\begin{equation}
...
\begin{equation}
\left(\frac{\rho^{*,n+1}}{\Delta t} - \half\mathcal{A}_0\right)\vb^{*,n+1} + \nabla\pi = \frac{\rho^n\vb^n}{\Delta t} + \nabla\cdot(-\rho^n\vb^n\vb^n) + \half\mathcal{A}_0\vb^n + \nabla\cdot\Sigmab^n.\label{eq:CN Vel Pred2}
\end{equation}
Relating this to the GMRES solver, we can see that we are solving for 
$(\xb_\vb,x_p) = (\vb^{*,n+1},\pi)$ with $b_p = -S^{*,n+1}$ and $\bb_\vb$ equal to the 
right-hand-side of (\ref{eq:CN Vel Pred2}).  For the Helmholtz-like operator, 
$\mathcal{A}=\Theta\alpha\mathcal{I} - \mathcal{A}_0$, we have $\Theta=1/\Delta t$, and $\alpha=\rho^{*,n+1}$,
noting that the coefficients
$\beta$ and $\gamma$ need to be multiplied by $\myhalf$.\\ \\
...
in the predictor.
\begin{eqnarray}
\frac{\rho^{n+1}\vb^{n+1} - \rho^n\vb^n}{\Delta t} + \nabla\pi &=& \half\nabla\cdot(-\rho^n\vb^n\vb^n - \rho^{n+1}\vb^{*,n+1}\vb^{*,n+1}) + \half(\mathcal{A}_0\vb^n + \mathcal{A}_0\vb^{n+1}) + \nabla\cdot\Sigmab^n,\nonumber\\
\label{eq:CN Vel Corr}
\end{eqnarray}
\begin{equation}
\nabla\cdot\vb^{n+1} = S^{n+1}.
\end{equation}
...
\begin{equation}
\left(\frac{\rho^{n+1}}{\Delta t} - \half\mathcal{A}_0\right)\vb^{n+1} + \nabla\pi = \frac{\rho^n\vb^n}{\Delta t} + \half\nabla\cdot(-\rho^n\vb^n\vb^n - \rho^{n+1}\vb^{*,n+1}\vb^{*,n+1}) + \half\mathcal{A}_0\vb^n + \nabla\cdot\Sigmab^n.
\end{equation}
\end{document}
