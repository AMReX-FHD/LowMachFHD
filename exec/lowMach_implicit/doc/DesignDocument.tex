...
Using
\begin{equation}
\Fb^n = \left[\rho^n\chi^n\left(\nabla c^n + K_P^n\nabla p^n\right)\right] + \underbrace{\sqrt{\frac{2\left(\chi\rho\mu_c^{-1}k_B T\right)^n}{\Delta t\Delta V}}\widetilde\Wb^n}_{\Psib^n},
\end{equation}
...
&&\hspace{-2.25in}\nabla\cdot(-\rho^n\vb^n\vb^n) + \half\left[\mathcal{A}_0^n\vb^n + \mathcal{A}_0^{n+1,*}(\vb^n + \deltab\vb)\right] + \nabla\cdot\underbrace{\sqrt{\frac{2(\eta k_B T)^n}{\Delta t\Delta V}}\widehat\Wb^n}_{\Sigmab^n} + \left(\frac{\rho^n+\rho^{n+1,*}}{2}\right)\gb,\nonumber\\
\end{eqnarray}
\begin{equation}
...
Using
\begin{equation}
\Fb^n = \left[\rho^n\chi^n\left(\nabla c^n + K_P^n\nabla p^n\right)\right] + \sqrt{\frac{2\left(\chi\rho\mu_c^{-1}k_B T\right)^n}{\Delta t\Delta V}}\widetilde\Wb^n,
\end{equation}
{\bf Step 2: Predictor Stokes Solve:}\\ \\
Solve a steady Stokes system with $p^{n+1,*} = p^n + \delta p$ and $\vb^{n+1,*} = \vb^n + \delta\vb$:
\begin{equation}
\nabla(p^n + \delta p) = \nabla\cdot\mathcal A_0^n(\vb^n + \deltab\vb) + \nabla\cdot\sqrt{\frac{2(\eta k_B T)^n}{\Delta t\Delta V}}\widehat\Wb^n + \rho^n\gb,
\end{equation}
\begin{equation}
\nabla\cdot(\vb^n + \delta\vb) = S^n,
\end{equation}
which can be written as
\begin{equation}
-\mathcal{A}_0^n\deltab\vb + \nabla\delta p = -\nabla p^n + \mathcal{A}_0^n\vb^n
+ \nabla\cdot\sqrt{\frac{2(\eta k_B T)^n}{\Delta t\Delta V}}\widehat\Wb^n 
...
\end{equation}
Relating this to the GMRES solver, we can see that we are solving for 
$(\xb_\vb,x_p) = (\deltab\vb,\delta p)$ with $b_p = \nabla\cdot\vb^n-S^n$ (note the change in sign!) 
...
\begin{eqnarray}
\rho^{n+1,*} &=& \rho^n + \Delta t\nabla\cdot(-\rho^n\vb^{n+1,*}),\\
\rho_1^{n+1,*} &=& \rho_1^n + \Delta t\nabla\cdot(-\rho_1^n\vb^{n+1,*} + \Fb^n).
\end{eqnarray}
...
+ \sqrt{\frac{2\left(\chi\rho\mu_c^{-1}k_B T\right)^{n+1,*}}{\Delta t\Delta V}}\widetilde\Wb^n
\end{equation}
{\bf Step 5: Corrector Stokes Solve:}\\ \\
Solve a steady Stokes system with $p^{n+1} = p^n + \delta p$ and $\vb^{n+1} = \vb^n + \delta\vb$:
\begin{equation}
\nabla(p^n + \delta p) = \nabla\cdot\mathcal A_0^{n+1,*}(\vb^n + \deltab\vb) + \nabla\cdot\sqrt{\frac{2(\eta k_B T)^{n+1,*}}{\Delta t\Delta V}}\widehat\Wb^n + \rho^{n+1,*}\gb,
\end{equation}
\begin{equation}
\nabla\cdot(\vb^n + \delta\vb) = S^{n+1,*},
\end{equation}
which can be written as
\begin{equation}
-\mathcal{A}_0^{n+1,*}\deltab\vb + \nabla\delta p = -\nabla p^n + \mathcal{A}_0^{n+1,*}\vb^n
+ \nabla\cdot\sqrt{\frac{2(\eta k_B T)^{n+1,*}}{\Delta t\Delta V}}\widehat\Wb^n 
+ \rho^{n+1,*}\gb,
\end{equation}
\begin{equation}
-\nabla\cdot\deltab\vb = \nabla\cdot\vb^n - S^{n+1,*}.
\end{equation}
{\bf Step 6: Trapezoidal Scalar Corrector:}\\ \\
Update the scalars using
\begin{eqnarray}
\rho^{n+1} &=& \half\rho^n + \half\left[\rho^{n+1,*} + \Delta t\nabla\cdot(-\rho^{n+1,*}\vb^{n+1})\right],\\
\rho_1^{n+1} &=& \half\rho_1^n + \half\left[\rho_1^{n+1,*} + \Delta t\nabla\cdot(-\rho_1^{n+1,*}\vb^{n+1} + \Fb^{n+1,*})\right].
\end{eqnarray}
