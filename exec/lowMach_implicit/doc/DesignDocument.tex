...
{\bf No-Slip Reservoir:} We specify $c$ at the boundary, and use the EOS to obtain 
$\rho$ at the
boundary.  The normal velocity must be equal to $-(\rho^{-1}\beta)F_n$ in order for
the EOS to remain satisfied.  The no-slip condition implies that $v_{\taub}=0$ on the 
boundary.\\

{\bf Free-Slip Reservoir:} This is the same situation as the no-slip reservoir, except
that $v_{\taub}$ are subject to (\ref{eq:free slip}).  Since in general
