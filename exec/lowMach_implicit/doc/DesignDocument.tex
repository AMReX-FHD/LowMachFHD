...
fluxes are both zero, $F_n=0$, as well as the advective flux, $\rho v_n = \rho_1 v_n = 0$.  The zero
...
we simply require $\partial v_{\tau}/\partial n=0$.\\
...
the EOS to remain satisfied.  The no-slip condition implies that each $v_{\tau}$ is
...
\begin{itemize}
\item The viscous operator for faces {\it on} the boundary must
evaluate to zero, so it does not matter what value is stored for the normal component
of velocity in the ghost face immediately outside of the valid region, unless you are concerned 
...
a separate face-centered multifab (or equivalently a multifab with two components) 
to handle boundary conditions at each transverse domain boundary.  
These multifabs do not require ghost cells either.
We use the interior cell against the boundary to represent
...
free-slip walls or free-slip reservoir.  This only matters if the normal velocity
