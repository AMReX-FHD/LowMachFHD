...
here is that if $q=0$ when using BDS to advect $\rho$, and $q=\nabla\cdot\Fb$, where
$\Fb$ is the same flux used to generate the RHS of the divergence constraint on the
velocity field, that the BDS advection scheme will ensure that the resulting edge
state $(\rho,\rho_1)$ satisfy the EOS.  There is a proof on our whiteboard here that 
