...
fluxes are both zero, $F_n=\rho\chi\partial c/\partial n+\Psi_n=0$, as well as the 
advective flux, $\rho v_n = \rho_1 v_n = 0$.
The zero
...
{\bf Free-Slip Wall:}  This is the same situation as the no-slip impermeable
...
this condition reduces to $\partial v_{\tau}/\partial n=0$.\\
...
use this to directly specify $v_n$ on boundaries.  Since we treat the densities
explicitly, we can keep these boundary conditions through the implicit velocity
update.  The no-slip condition implies that each $v_{\tau}$ is
...
boundary.  To deal with normal velocities, the multifab {\tt vel\_bc\_n(nlevs,dm)}
stores the actual Dirichlet value.  The $d^{\rm th}$ element is a face-centered
multifab in direction $d$.\\

To deal with transverse velocities, the multifab {\tt vel\_bc\_t(nlevs,X)} stores
either the Dirichlet or Neumann condition, depending on the type.  In 2D, X=2,
