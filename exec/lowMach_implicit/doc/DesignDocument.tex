...
Initialize $\vb^{\rm init}, \rho^0, \rho_1^0, \chi^0, K_P^0, \eta^0, \kappa^0$, and $\pi^0$.
Then, perform projection to obtain an initial velocity field, $\vb^0$, that satisfies
\begin{equation}
\nabla\cdot\vb^0 = S^0 \equiv S(\Fb^0); \qquad 
\Fb^0 = \left[\rho\chi\left(\nabla c + K_P\nabla p_0\right)\right]^0 + \underbrace{\sqrt{\frac{2\left(\chi\rho\mu_c^{-1}k_B T\right)^0}{\Delta t\Delta V}}\widetilde\Wb^0}_{\Psib^0}.
\end{equation}
...
Using
\begin{equation}
\Fb^n = \left[\rho\chi\left(\nabla c + K_P\nabla p_0\right)\right]^n + \underbrace{\sqrt{\frac{2\left(\chi\rho\mu_c^{-1}k_B T\right)}{\Delta t\Delta V}}\widetilde\Wb^n}_{\Psib^n},
\end{equation}
...
\qquad
\Fb^{*,n+1} = \left[\rho\chi\left(\nabla c + K_P\nabla p_0\right)\right]^{*,n+1} + \Psib^n.
\end{equation}
...
S^{n+1} \equiv S(\Fb^{n+1});
\qquad
\Fb^{n+1} = \left[\rho\chi\left(\nabla c + K_P\nabla p_0\right)\right]^{n+1} + \underbrace{\sqrt{\frac{2\left(\chi\rho\mu_c^{-1}k_B T\right)^{n+1}}{\Delta t\Delta V}}\widetilde\Wb^{n+1}}_{\Psib^{n+1}}.
\end{equation}
