...
In order for the evolution of the system to satisfy the equation of state,
\begin{equation}
\sum_i \frac{\rho_i}{\bar\rho_i} = 1,
\end{equation}
There velocity field used in the mass update,
\begin{equation}
\frac{\partial\rho_i}{\partial t} = -\nabla\cdot(\rho_i\vb) - \nabla\cdot\Fb_i,
\end{equation}
...
(see, e.g., {\bf Step 2}), so the best we can do is used an iteratively-lagged
...
\subsection{Implicit Potential Algorithm (New)}
We use an iterative approach, and then define a state at $t^n$, and
a $t^{n,k}$ state for velocities and densities.  The initial time-advanced
state is a copy of the state at $t^n$.\\ \\
{\bf Step 0: Initialization}\\ \\
Here $n=0$.
Begin with an initial guess for velocity, $\vb^{\rm init}$, and pressure, $p^0$.
Then, perform a projection to obtain an initial velocity field, $\vb^0$ that satisfies
...
\end{equation}
where $\Fb_i^n$ and $\nabla\cdot\Fb_i^n$ are computed from $\rho_i^n$ using the 
supplied {\tt compute\_mass\_fluxdiv} subroutine.
For the projection, we solve for $\phi$ and update $\vb^{\rm init}$ as follows:
\begin{equation}
...
We loop over {\bf Step 1} -- {\bf Step 3} for $l=0,\cdots$:\\ \\
{\bf Step 1: Density Update}\\ \\
\begin{equation}
\frac{\rho^{n+1,l+1}-\rho^n}{\Delta t} = -\half\nabla\cdot(\rho\vb)^n - \half\nabla\cdot(\rho\vb)^{n+1,l}
\end{equation}
{\bf Step 2: Concentration Update}\\ \\
We would like to solve the following,
\begin{eqnarray}
\frac{(\rho\wb)^{n+1,l+1} - (\rho\wb)^n}{\Delta t} + \half\nabla\cdot(\rho\wb\vb)^n + \half\nabla\cdot(\rho\wb\vb)^{n+1,l} &=&\half\nabla\cdot(\rho\Wb\chib\Gammab\nabla\xb)^n + \half\nabla\cdot(\rho\Wb\chib\Gammab\nabla\xb)^{n+1,l} \nonumber\\
&&\hspace{-2in} + \theta\nabla\cdot \Ab_\Phi^{n+1,l}\nabla\Phib^{n+1,l+1} +
(1-\theta)\nabla\cdot \Ab_\Phi^n\nabla\Phib^n\nonumber\\
&&\hspace{-2in} + \half\sqrt{\frac{2k_B}{\Delta t\Delta V}}\Lb_\half^n\mathcal{Z}^n
+ \half\sqrt{\frac{2k_B}{\Delta t\Delta V}}\Lb_\half^{n+1,l}\mathcal{Z}^n,
\end{eqnarray}
\begin{equation}
-\nabla\cdot\epsilon\nabla\Phib^{n+1,l+1} = (\rho\wb^T\zb)^{n+1,l+1},
\end{equation}

{\bf Step 3: Velocity Solve}\\ \\
