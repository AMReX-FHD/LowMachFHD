...
\begin{equation}
\frac{\partial n_k}{\partial t} = \nabla\cdot\left(D_k\nabla n_k + \sqrt{\frac{2D_kn_k}{A}}\mZb\right) + \mRb(\nb),
\end{equation}
...
Here, $A$ is the volume adjusting factor in general and the cross section of the system in the two dimensional case.
Hence, for a cell having dimensions $\Delta x\times\Delta y\times\Delta z$ (or $\Delta x\times\Delta y$), its volume is defined as $\Delta V=A\Delta x\Delta y\Delta z$ (or $\Delta V=A\Delta x\Delta y$). 
