       ...
       do reaction=1, nreactions
          ...
          do species=1, nspecies
             ...
             ! where N is the total number of molecules
                ...
                case(1) ! Rate ~ N
                   if (n_nonneg(species)*dv .ge. 1.d0) then
                ...
                case(2) ! Rate ~ N*(N-1)
                   if (n_nonneg(species)*dv .ge. 2.d0) then
                      reaction_rates(reaction) = reaction_rates(reaction)*n_nonneg(species)*(n_nonneg(species)-1.d0/dv)
                   else
                      reaction_rates(reaction) = 0.d0
                   end if

                case(3) ! Rate ~ N*(N-1)*(N-2)
                   if (n_nonneg(species)*dv .ge. 3.d0) then
                      reaction_rates(reaction) = &
                           reaction_rates(reaction)*n_nonneg(species)*(n_nonneg(species)-1.d0/dv)*(n_nonneg(species)-2.d0/dv)
                   else
                      reaction_rates(reaction) = 0.d0
                   end if
