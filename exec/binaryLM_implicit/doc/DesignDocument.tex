...
\begin{equation}
u = \pm\half\left[1 + \sin{(2\pi x - \half\pi)}\right].
\end{equation}
The x-walls are stationary no-slip.  The time step for the coarsest simulation is
$\Delta t = 1.6\times 10^{-2}$ and is reduced by a factor of 2 as the grid spacing
decreases by a factor of 2.  We run each simulation to 0.8 seconds.
We use $\bar\rho_1 = 2, \bar\rho_2 = 1$.  The initial conditions are $\vb=0$ 
and $p=0$ with
a Gaussian bump of high density centered at $(x,y) = (0.5, 0.75)$ such that
\begin{equation}
c = 1 - \half\left[1 + \tanh\left(32r-2.5\right)\right]; \quad r = \sqrt{x^2 + (y-0.75)^2}
\end{equation}
...
$\chi=10^{-3}$ when $c=1$.  Gravity acts downward such that $\gb=(0,-10)$.

Viscous operator form $\nabla + \nabla^T$

Test 1: centered advection, inertial algorithm
Test 2: centered advection, overdamped algorithm
Test 3: BDS advection, inertial algorithm
Test 4: BDS advection, overdamped algorithm
Test 5: BDS advection, inertial algorithm, $\chi=0$
Test 6: BDS advection, overdamped algorithm, $\chi=0$
