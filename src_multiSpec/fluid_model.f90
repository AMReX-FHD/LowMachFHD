module fluid_model_module
  ...
  public :: fluid_model, compute_D_barGama_local
...
contains
  ...
  subroutine fluid_model(mla,rho,rhotot,molarconc,molmtot,D_bar,D_therm,Gama,the_bc_level)
    ...
    do n=1,nlevs
       do i=1,nfabs(rho(n))
          ...
          select case(dm)
          case (2)
             call compute_D_barGama_2d(dp(:,:,1,:),dp1(:,:,1,1),dp2(:,:,1,:),&
                  dp3(:,:,1,1),dp4(:,:,1,:),dp5(:,:,1,:),dp6(:,:,1,:),ng,lo,hi) 
          case (3)
             call compute_D_barGama_3d(dp(:,:,:,:),dp1(:,:,:,1),dp2(:,:,:,:),&
  ...
  subroutine compute_D_barGama_2d(rho,rhotot,molarconc,molmtot,D_bar,D_therm,Gama,ng,lo,hi)
    ...
    do j=lo(2)-ng,hi(2)+ng
       do i=lo(1)-ng,hi(1)+ng
          ...
          call compute_D_barGama_local(rho(i,j,:),rhotot(i,j),molarconc(i,j,:),&
                                      molmtot(i,j),D_bar(i,j,:),D_therm(i,j,:),Gama(i,j,:))
       end do
    end do
   
  end subroutine compute_D_barGama_2d

  subroutine compute_D_barGama_3d(rho,rhotot,molarconc,molmtot,D_bar,D_therm,Gama,ng,lo,hi)
    ...
    do k=lo(3)-ng,hi(3)+ng
       do j=lo(2)-ng,hi(2)+ng
          do i=lo(1)-ng,hi(1)+ng
             ...
             call compute_D_barGama_local(rho(i,j,k,:),rhotot(i,j,k),molarconc(i,j,k,:),&
                                         molmtot(i,j,k),D_bar(i,j,k,:),D_therm(i,j,k,:),Gama(i,j,k,:))
          end do
       end do
    end do
   
  end subroutine compute_D_barGama_3d
  ...
  subroutine compute_D_barGama_local(rho,rhotot,molarconc,molmtot,D_bar,D_therm,Gama)
    ...
    ! populate D_bar, H, I and X_xxT where  X = molmtot*W*M^(-1) 
    n=0; 
    do row=1, nspecies  
       do column=1, row-1
          n=n+1
  ...
  end subroutine compute_D_barGama_local
