...
contains
  ...
  subroutine eos_model_init()
    ...
    do ns=1,nspecies
       ...
       b_i(ns) = Runiv*Tcrit(ns)/(8.d0*molecular_weight(ns)*Pcrit(ns))
  ...
  ! scalar a
  subroutine compute_a(a,Yk)

    real(kind=8) :: a,Yk(1:nspecies)

    integer :: i,j
    real(kind=8) :: a_i(1:nspecies)
    
    call compute_a_i(a_i)
    a = 0.d0
    do j=1,nspecies
       do i=1,nspecies
  ...
  subroutine compute_a_i(a_i)
    ...
    do n=1,nspecies
       a_i(n) = (27.d0/64.d0) * Runiv**2*Tcrit(n)**2 / (molecular_weight(n)**2 * Pcrit(n))
    end do
  ...
  subroutine compute_dadw(dadw,Yk)

    real(kind=8) :: dadw(1:nspecies),Yk(1:nspecies)
  ...
  subroutine CKPY(rho, temp, Yk, iwrk, rwrk, pt) 
    ...
    pt = (Runiv*temp*molmixinv) / (rhoinv - b) - a / rhoinv**2
  ...
  subroutine CKRHOY(pt,temp,Yk,IWRK,RWRK,rho)
    ...
    do i=1,10
       f = a*b*rho**3 - a*rho**2 + (c+pt*b)*rho - pt
       fprime = 3.d0*a*b*rho**2 - 2.d0*a*rho + c+pt*b
       rho = rho - f/fprime
    end do
